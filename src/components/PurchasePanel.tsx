...
export default function PurchasePanel() {
  ...
  return (
    <div className="mt-6 sm:mt-10">
      ...
      <p className="mt-3 max-w-[46ch] text-[14.5px] leading-[1.6] text-graphite sm:mt-4 sm:text-[16px]">
        {product.subhead}
      </p>

      {/* Benefit micro-row */}
      <ul className="mt-5 grid grid-cols-3 gap-2 border-y border-sand/60 py-3.5 sm:mt-7 sm:py-5">
        {product.benefits.map((benefit) => {
          const Icon = icons[benefit.icon];
          return (
            <li key={benefit.label} className="flex flex-col items-center gap-2 text-center">
              <span className="h-[22px] w-[22px] text-gold">
                <Icon />
              </span>
              <span className="text-[10px] font-medium uppercase leading-tight tracking-[0.12em] text-graphite sm:text-[10.5px] sm:tracking-eyebrow">
                {benefit.label}
              </span>
            </li>
          );
      ...
      <div className="mt-5 rounded-xl2 border border-white/70 bg-white/70 p-5 shadow-soft backdrop-blur-sm sm:mt-7 sm:p-6">
        <div className="flex items-center gap-2">
          <DiamondIcon className="h-2.5 w-2.5 text-gold" />
          <p className="text-[10px] font-semibold uppercase tracking-eyebrow text-gold sm:text-[10.5px]">
            {product.offer.label}
          </p>
        </div>
        ...
        <div className="mt-4 flex flex-wrap items-end justify-between gap-x-4 gap-y-3">
          ...
          {savings && (
            <span className="rounded-full bg-charcoal px-3.5 py-1.5 text-[10.5px] font-semibold uppercase tracking-wide2 text-ivory">
              Save {savings}% today
            </span>
          )}
        </div>
                ...
                return (
                  <label
                    ...
                    className={`relative flex cursor-pointer flex-col rounded-2xl border px-3.5 py-2.5 transition-all duration-300 ease-silk ${
                      selected
                        ? "border-gold bg-champagne/40 shadow-soft"
                        : "border-sand/70 bg-white/60 hover:border-goldsoft"
                    }`}
                  >
                    <input
                      type="radio"
                    ...
                    />
                    {option.badge && (
                      <span className="absolute -top-2 right-3 rounded-full bg-charcoal px-2 py-[3px] text-[9.5px] font-semibold uppercase tracking-wide2 text-ivory">
                        {option.badge}
                      </span>
                    )}
                    <span className="font-serif text-[18px] leading-none text-charcoal">
                      {option.title}
                    </span>
                    {option.note && (
        ...
        <p className="mt-3 flex items-center justify-center gap-1.5 text-[10.5px] uppercase tracking-eyebrow text-gold">
      ...
      <div className="mt-6 flex items-center justify-center gap-2.5">
        <Stars />
        <span className="text-[11.5px] tracking-wide text-graphite">
