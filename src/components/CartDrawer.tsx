...
export default function CartDrawer() {
  ...
  return (
    ...
    >
      ...
      <div
        onClick={closeCart}
        className={`absolute inset-0 bg-charcoal/25 backdrop-blur-[2px] transition-opacity duration-400 ease-silk ${
      ...
      <div
        ...
        aria-modal="true"
        aria-label="Your bag"
        className={`absolute inset-y-0 right-0 flex w-full max-w-[420px] flex-col border-l border-white/70 bg-ivory shadow-lift transition-transform duration-450 ease-silk ${
          isOpen ? "translate-x-0" : "translate-x-full"
        }`}
      >
        <div className="flex items-center justify-between border-b border-sand/60 px-5 py-4">
          <h2 className="font-serif text-[19px] tracking-wide text-charcoal">
            Your Bag
          ...
          <button
            ...
            className="flex h-9 w-9 items-center justify-center rounded-full text-charcoal transition-colors duration-200 hover:bg-champagne/60"
          >
            <CloseIcon />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto overscroll-contain px-5 py-5">
          {lines.length === 0 ? (
            <div className="flex h-full flex-col items-center justify-center text-center">
              ...
              <p className="mt-5 font-serif text-[20px] text-charcoal">Your bag is empty.</p>
            ...
            <ul className="space-y-4">
              {lines.map((line) => {
                ...
                return (
                  ...
                  >
                    <div className="relative h-[86px] w-[70px] shrink-0 overflow-hidden rounded-xl bg-linen">
                      {thumb ? (
                        <Image
                          src={thumb.src}
                          alt={thumb.alt}
                    ...
                    <div className="flex min-w-0 flex-1 flex-col">
                      <div className="flex items-start justify-between gap-2">
                        ...
                        <button
                          ...
                          className="-mr-1 -mt-1 flex h-7 w-7 shrink-0 items-center justify-center rounded-full text-muted transition-colors duration-200 hover:bg-champagne/60 hover:text-charcoal"
                        >
                      ...
                      <div className="mt-auto flex items-end justify-between gap-2 pt-3">
                        ...
                        <div className="text-right">
                          <p className="text-[14px] font-medium leading-none text-charcoal">
        ...
        {lines.length > 0 && (
          <div className="border-t border-sand/60 bg-white/60 px-5 pt-4 backdrop-blur-sm pb-safe">
            {savedAmount > 0 && (
              <div className="mb-2.5 flex items-center justify-between text-[12px]">
                <span className="text-muted">Launch offer savings</span>
                <span className="font-medium text-gold">−{formatPrice(savedAmount)}</span>
              </div>
            )}

            <div className="flex items-baseline justify-between">
              <span className="text-[12px] uppercase tracking-eyebrow text-muted">Subtotal</span>
              <span className="font-serif text-[26px] leading-none text-charcoal">
            ...
            {checkoutError && (
              <p
                role="alert"
                className="mt-3 rounded-xl border border-goldsoft bg-white/70 px-3 py-2 text-[11.5px] leading-snug text-graphite"
              >
                {checkoutError}
              </p>
            )}
